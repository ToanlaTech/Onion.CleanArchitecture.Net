...
function convert_block_three(number: string) {
  ...
  switch (_a.length) {
    ...
    case 3:
      ...
      var tram = chuHangTram[Number(_a[0])] + ' trăm';
      return tram + ' ' + chuc_dv;
  }
}

function convert_block_two(number: string) {
  var dv = chuHangDonVi[Number(number[1])];
  var chuc = chuHangChuc[Number(number[0])];
  var append = '';
...
export function readMoney(number: number) {
